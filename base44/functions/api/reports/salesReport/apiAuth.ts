...
export async function requirePermission(base44, tenantUser, permission) {
  ...
  if (!hasPermission) return forbidden('Permission required: ' + permission);
  return null;
}
export function requireAdmin(tenantUser) {
  if (!tenantUser.is_owner && tenantUser.role_name !== 'Admin') return forbidden('Admin access required');
  return null;
}
