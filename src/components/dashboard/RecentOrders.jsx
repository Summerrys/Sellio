            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500">
                  {order.items?.length || 0} {order.items?.length === 1 ? 'item' : 'items'} · ${order.total_amount?.toFixed(2)}
                </p>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                <p className="text-xs text-slate-400">{format(new Date(order.created_date), 'h:mm a')}</p>
