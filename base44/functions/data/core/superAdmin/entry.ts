...
const DATA_FILE = '/tmp/data_core_super_admin.json';
...
function validate(data, isUpdate = false) {
  ...
  if (data.role && !['admin', 'support', 'developer'].includes(data.role)) {
...
Deno.serve(async (req) => {
  try {
    ...
    if (operation === 'read') {
      ...
      if (!record) {
        return Response.json({ error: 'Record not found', entity: 'super_admin', id }, { status: 404 });
      }
