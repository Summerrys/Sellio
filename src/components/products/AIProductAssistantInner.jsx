...
function StockImageSearch({ onResult, onError }) {
  ...
  const doSearch = async () => {
    ...
    try {
      const result = await base44.integrations.Core.InvokeLLM({
        prompt: `Find a high-quality product photo URL for: "${q}". Return a single direct image URL (ending in .jpg, .jpeg, .png, or .webp) from a reputable source like Unsplash or Pexels. The image should be clean, well-lit, and suitable for an e-commerce product listing. Return ONLY the raw URL with no explanation, no markdown, nothing else.`,
        add_context_from_internet: true,
      });
      const text = typeof result === 'string' ? result : JSON.stringify(result);
      const urlMatch = text.match(/https?:\/\/[^\s"'<>]+\.(jpg|jpeg|png|webp)(\?[^\s"'<>]*)?/i);
      if (urlMatch) {
        setResultUrl(urlMatch[0]);
      } else {
...
export default function AIProductAssistantInner({ onApply, tenantId, businessType, currency, categories, currentImageUrl, onImageChange, onAdditionalImagesChange, additionalImagesOnOpen, onImageDelete, onCategoriesRefresh, currentProductName, innerRef }) {
  ...
  // Expose state to parent via innerRef
  useEffect(() => {
    if (innerRef) {
      innerRef.current = {
        deletedImagesRef,
  ...
  const buildPermanentPath = (tid, filename) => {
    const safeName = filename.replace(/[^a-zA-Z0-9._-]/g, '_');
    return `${tid}/products/${Date.now()}-${safeName}`;
  };
  ...
  return (
    <>
      <div className={cn(
          ...
          <div className="flex items-center gap-4 p-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="mt-2 h-1.5 bg-slate-200 rounded-full overflow-hidden">
                <div className="h-full rounded-full transition-all duration-500" style={{ width: step === 'uploading' ? '40%' : '85%', background: themeColor }} />
              </div>
            </div>
          </div>
        )}
          ...
          <div className="p-3">
            <div className="grid grid-cols-4 gap-3">
              <div
                ...
                onClick={() => { setEditingImageIndex(null); setEditModalOpen(true); }}
              >
              ...
              {additionalImages.map((src, idx) => (
                <div
                  ...
                  onClick={() => { setEditingImageIndex(idx); setEditModalOpen(true); }}
                >
        ...
        <input ref={replaceImageInputRef} type="file" accept="image/*" onChange={handleReplaceImageSelect} className="hidden" />
      </div>
      ...
      {editModalOpen && (
        <ImageEditModal
          ...
          onClose={() => { setEditModalOpen(false); setEditingImageIndex(null); }}
