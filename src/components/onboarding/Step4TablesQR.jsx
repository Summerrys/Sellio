...
import QRPreviewModal from '../tables/QRPreviewModal';
  ...
  return (
    <Card className="p-3 sm:p-5 bg-white border-0 shadow-lg w-full" style={{ maxWidth: '100%', boxSizing: 'border-box' }}>
      ...
      {selectedQR && qrModalOpen && (
        <QRPreviewModal
          table={selectedQR}
          qrUrl={qrCodes[selectedQR.id]}
          onClose={() => { setQRModalOpen(false); setSelectedQR(null); }}
          onDownload={() => {
            const link = document.createElement('a');
            link.href = qrCodes[selectedQR.id];
            link.download = `${selectedQR.name.replace(/\s+/g, '_')}_QR.png`;
            link.click();
          }}
          subtitle={`${selectedQR.capacity} pax`}
