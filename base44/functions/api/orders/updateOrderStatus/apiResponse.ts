/** Local copy: Base44 functions bundle only files inside their own function folder. */
...
export function validationError(message = 'Validation failed', details = []) {
  return Response.json({ success: false, error: { code: 'VALIDATION_ERROR', message, details } }, { status: 422 });
}
export function paginated(items, { limit, cursor, hasMore }) {
  return success(items, { pagination: { limit, cursor, hasMore, count: items.length } });
}
