...
import {
  Building2, MapPin, Camera, X, Save, Percent, Loader2, Pencil, QrCode, RefreshCw
} from 'lucide-react';
...
export default function BusinessProfileTab({ tenant, tenantId }) {
  ...
  const tabs = [
    ...
    { key: 'payment_qr', label: 'Payment QR' },
  ];

  return (
    <div className="max-w-2xl space-y-6">
      {/* Tab switcher */}
      <div className="flex w-full rounded-xl border border-slate-100 bg-slate-50 p-1 gap-1">
        {tabs.map(tab => (
          <button
            key={tab.key}
            ...
            style={activeTab === tab.key
              ? { background: 'var(--color-primary-gradient, #6366f1)', color: '#fff', boxShadow: '0 1px 4px rgba(0,0,0,0.12)' }
          ...
          <div className="flex flex-col items-center gap-5">
            <div className="w-full text-center">
              ...
              <p className="text-xs text-slate-400">
                Upload your PayNow, Touch N Go, DuitNow or any payment QR.<br />Shown to customers after they place an order.
              </p>
            </div>
            ...
            {paymentQRPreview ? (
              <div className="flex flex-col items-center gap-2">
                ...
                >
                  ...
                  {/* Replace button — center overlay, always visible on mobile, hover on desktop */}
                  <button
                    type="button"
                    onClick={() => paymentQRInputRef.current?.click()}
                    className="absolute inset-0 flex items-center justify-center transition-opacity"
                    style={{ opacity: qrHovered ? 1 : window.matchMedia?.('(hover: none)').matches ? 1 : 0 }}
                  >
